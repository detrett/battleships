...
describe("GameController", () => {
  ...
  beforeEach(() => {
    ...
    playerTwo = new Player("Player 2", "ai");
    gameController = new GameController(playerOne, playerTwo);
  });

  test("initializes with correct players", () => {
    expect(gameController.players).toHaveLength(2);
    expect(gameController.activePlayer).toBe(playerOne);
  });

  test("switches player turns correctly", () => {
    const initialPlayer = gameController.getActivePlayer();
    gameController.switchPlayerTurn();
    expect(gameController.getActivePlayer()).not.toBe(initialPlayer);
    expect(gameController.getActivePlayer()).toBe(playerTwo);
  });

  test("gets inactive player correctly", () => {
    expect(gameController.getInactivePlayer()).toBe(playerTwo);
    gameController.switchPlayerTurn();
    expect(gameController.getInactivePlayer()).toBe(playerOne);
  });

  test("plays round correctly", () => {
    const mockGameboard = {
      receiveAttack: jest.fn().mockReturnValue(false),
      checkForGameOver: jest.fn().mockReturnValue(false),
    };

    playerTwo.gameboard = mockGameboard;
    gameController.playRound(0, 0);

    expect(mockGameboard.receiveAttack).toHaveBeenCalledWith(0, 0);
    expect(mockGameboard.checkForGameOver).toHaveBeenCalled();
  });

  test("handles game over condition", () => {
    const mockGameboard = {
      receiveAttack: jest.fn().mockReturnValue(false),
      checkForGameOver: jest.fn().mockReturnValue(true),
    };
    ...
    expect(consoleSpy).toHaveBeenCalledWith(
      "Game Over! All of Player 2's ships have been sunk!"
    );
  });

  test("plays AI round correctly", () => {
    const mockGameboard = {
      receiveAttack: jest.fn().mockReturnValue(false),
      checkForGameOver: jest.fn().mockReturnValue(false),
    };

    playerOne.gameboard = mockGameboard;
